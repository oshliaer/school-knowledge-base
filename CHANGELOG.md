...
### Добавлено
- `docs/FOR_TEACHERS.md` — страница для учителей (как использовать колоды с классом, доступные материалы)
- `docs/DEVELOPMENT.md` — техническая документация (команды, форматы карточек, структура проекта, CI/CD)
- `docs/ARCHITECTURE.md` — архитектура проекта (описание пайплайнов, структура репозитория, известные ограничения)

### Изменено
- `README.md` — переориентирован на школьников/потребителей (добавлены наглядные примеры, 3 шага для быстрого старта, ссылки на релизы и страницу для учителей)
- `CLAUDE.md` — добавлен явный project statement в начало
