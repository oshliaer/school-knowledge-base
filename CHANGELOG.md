...
### Добавлено
- Новый обязательный разделитель `^^^` между вопросом и ответом в карточках (без него карточка пропускается при сборке)
...
### Изменено
- Формат карточек с явным разделением вопроса и ответа через `^^^` (breaking change)
...
### Удалено
- Поддержка старого формата карточек без разделителя `^^^` (breaking change)
